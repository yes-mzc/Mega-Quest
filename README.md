...
│   ├── shopItems.ts         # 기프트콘 상품 8개
│   ├── quickLinks.ts        # 바로가기 링크 5개
...
상품 추가/수정은 `lib/shopItems.ts`만 편집하면 됩니다.
...
| 전체 진행률 | 퀘스트 목록 상단에 `N/35 완료` 배너 표시 |
...
**`'use client'` 필수 컴포넌트**  
`useRouter`, `useGameStore`, `useState` 등 클라이언트 훅을 사용하는 모든 컴포넌트에 반드시 선언해야 합니다. `QuickLinks.tsx`는 정적 링크만 렌더링하므로 Server Component로 유지합니다.
